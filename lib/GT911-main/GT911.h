...
public:
  GT911(TwoWire *twi = &Wire, int8_t intPin = -1, uint8_t intNum = -1, int8_t rstPin = -1, uint8_t mode = GT911_MODE_INTERRUPT, uint8_t addr = GT911_I2C_ADDR_BA, uint32_t clk = 400000);
  bool begin();
  bool productID(uint8_t *buf, uint8_t len);
  GTConfig *readConfig();
  GTConfig *getReadedConfig();
  bool writeConfig();
  GTInfo *readInfo();
